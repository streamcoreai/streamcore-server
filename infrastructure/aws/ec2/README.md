...
- **Docker Compose** running both Caddy and the VoiceAgent server
...
| `CONFIG_TOML` | Full contents of your production `config.toml` (with real API keys) |
...
4. **Start** — runs `docker compose up -d` (Caddy + VoiceAgent server)
