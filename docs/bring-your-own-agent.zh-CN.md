...
**`text` 永远只是用户实际说出的话。** 服务端不会把上下文塞进去。这一点对任何会持久化收到内容的 agent 都很关键：如果检索结果和打断提示被拼进消息里，你的对话历史里存下的就会是 `[Context: …] User: 我的余额是多少`，而不是 `我的余额是多少`，之后每一轮都会把这串东西当成用户的原话再读一遍。

上下文改为并列传递，且每个字段都是可选的：

| 字段 | 何时出现 | 该怎么用 |
|---|---|---|
| `interrupted_text` | 用户插话打断了 agent 上一次回复 | 用户实际听到的部分。可用来避免重复，也可用来订正你存下的内容——用户只听到了这些，而不是你生成的整段回复 |
| `context` | 启用了 `[rag]` 且本轮命中 | 检索到的片段。可拼进你的提示词，若你的 agent 自己做检索则可忽略 |
| `summary` | 长通话中滚动摘要开始生效后 | 服务端对更早若干轮的摘要。如果你的 agent 自带记忆，它基本是冗余的 |
| `system` | 配置了技能，或本轮语音识别质量差 | 是指令而非内容。请前置到你的 system prompt |
